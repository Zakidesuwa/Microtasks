<script lang="ts">
  ...
  interface CalendarEvent {
    id: string; // This will be the Task ID
    title: string;
    description?: string;
    color?: string;
  ...
  let eventDate: string | null = null; // For the form input type="date", format YYYY-MM-DD
    ...
    for (let i = 1; i <= daysInCurrentActualMonth; i++) {
                ...
                if (task.dueDateISO) {
                    ...
                    if (taskYear === currentDateObj.getFullYear() &&
                        ...
                        eventsForThisDay.push({
                            id: task.id, // Store task.id here
                            title: task.title,
                            description: task.description,
                            deadlineTime: task.dueTime || undefined,
        ...
        if (isTodayFlag && today.getDay() === 4) { // 4 for Thursday
            const thursdaySpecialEvents: CalendarEvent[] = [
                { id: 'today_thu_evt1', title: 'Project Deadline Meeting', deadlineTime: '11:30', color: '#EF4444' }, // Use HH:MM for consistency
                { id: 'today_thu_evt2', title: 'System Security Update', deadlineTime: '16:00', color: '#F59E0B' }, // Use HH:MM for consistency
            ];
  ...
  function openAddEventForm(dayDateObj?: Date) {
    ...
    eventDueTime = null; // Keep as null, form input handles it
  ...
  function openTaskDetailsModal(eventId: string) {
    // Check if it's one of the hardcoded Thursday events
    if (eventId.startsWith('today_thu_evt')) {
        const specialEvent = daysInMonth.flatMap(d => d.events).find(e => e.id === eventId);
        if (specialEvent) {
            // For special events, we create a temporary TaskForFrontend-like object
            // or adapt the modal to handle CalendarEvent directly.
            // Here, we'll make it compatible with selectedTaskForDetails:
            selectedTaskForDetails = {
                id: specialEvent.id,
                title: specialEvent.title,
                description: specialEvent.description || "This is a special recurring event for Thursdays.",
                isCompleted: false, // Or determine based on time if relevant
                status: 'pending', // Or other appropriate status
                priority: 'standard',
                createdAtISO: null, // Not applicable
                dueDateISO: new Date(currentYear, currentMonth, daysInMonth.find(d => d.events.some(e => e.id === eventId))?.dayNum || 1).toISOString().split('T')[0],
                dueTime: specialEvent.deadlineTime || null, // Ensure this is HH:MM
                color: specialEvent.color || '#3B82F6'
            };
    ...
    if (task) {
      ...
      console.warn(`Task with ID ${eventId} not found in data.tasks for details modal.`);
    }
  }
  ...
  const eventFormEnhanceCallback = () => {
    ...
    return async ({ result, update }: { result: any, update: any }) => {
      if (result.type === 'success' ) {
        closeEventForm();
        // Invalidate to refetch data, which will trigger reactive updateCalendarDays
        await invalidateAll();
        // No need to call updateCalendarDays manually if it's reactive to `data.tasks`
      ...
      // We call update to apply form data back if there's an error,
      // or to clear the form on success if not handled by closeEventForm's variable resets.
      // `reset: false` means it won't clear the form fields automatically, which is good for error display.
      // On success, our closeEventForm should clear fields.
      await update({ reset: result.type !== 'failure' && result.type !== 'error' });
    };
  };

  const handleEscKey = (event: KeyboardEvent) => {
    if (event.key === 'Escape') {
      if (showTaskDetailsModal) closeTaskDetailsModal();
      else if (showEventForm) closeEventForm();
  ...
  // Reactive statement to update username and calendar when data changes
  $: if (browser) {
    if (data.user?.name) {
      username = data.user.name;
    } else {
      username = getStoredUsername(); // Fallback if user data isn't fully loaded
    }
    pageError = data.error || null; // Update page error from data
    if (data.tasks) { // Ensure tasks are present before updating calendar
        updateCalendarDays();
    }
  }
  ...
  onMount(() => {
    ...
    const handleGlobalClick = (event: MouseEvent) => {
      ...
      if (!isClickInsideDropdownTrigger && !isClickInsideDropdownWindow) {
        closeOtherWindows(''); // Pass empty string to close all
      }
    };
    ...
    // Initial calendar update is now handled by the reactive block based on `data.tasks`
    // However, if data.tasks might not be initially populated but available,
    // a direct call here or ensuring the reactive block runs is key.
    // The reactive block `$: if (browser && data.tasks)` should cover this.
    if (browser && data.tasks) {
        updateCalendarDays(); // Ensure initial load if data is already there
    }


    return () => {
      console.log('[Calendar Page / +page.svelte] Component Unmounting');
      clearInterval(intervalId);
      document.removeEventListener('click', handleGlobalClick);
      document.removeEventListener('keydown', handleEscKey);
      // Clean up icon listeners if necessary, though Svelte might handle this for component elements
      const bellIcon = document.getElementById('bellIcon');
      if (bellIcon) bellIcon.onclick = null; // Or removeEventListener
...
<div class={`flex h-screen font-sans ${isDarkMode ? 'dark bg-zinc-900 text-zinc-300' : 'bg-gray-100 text-gray-800'}`}>
    ...
    >
      <div>
        <div class="flex items-center gap-2 mb-8 pb-4 border-b ${isDarkMode ? 'border-zinc-700' : 'border-gray-200'}">
          <!-- Microtask Logo reverted to img -->
        ...
        <nav class="flex flex-col gap-2">
          <a href="/home" class="flex items-center gap-3 px-3 py-2 rounded-md font-semibold transition-colors duration-150" class:hover:bg-gray-100={!isDarkMode} class:hover:bg-zinc-700={isDarkMode} class:text-gray-700={!isDarkMode} class:text-zinc-300={isDarkMode}>
          ...
          <a href="/calendar" class="flex items-center gap-3 px-3 py-2 rounded-md font-semibold transition-colors duration-150" class:bg-blue-600={!isDarkMode} class:bg-blue-800={isDarkMode} class:text-white={true} class:hover:bg-gray-100={!isDarkMode} class:hover:bg-zinc-700={isDarkMode}>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-5 h-5" aria-hidden="true">
    ...
    <div class="flex-1 overflow-y-auto pt-[60px] pb-4 flex flex-col">
        <div class={`w-full px-4 sm:px-6 py-3 flex items-center justify-between gap-3 shadow-sm flex-shrink-0 border-b ${isDarkMode ? 'bg-zinc-800 text-zinc-100 border-zinc-700' : 'bg-white text-gray-800 border-gray-200'}`}>
            ...
            <button on:click={() => openAddEventForm()} class={`px-3 py-1.5 text-sm font-medium rounded-md flex items-center gap-2 transition-colors ${isDarkMode ? 'bg-blue-700 hover:bg-blue-600 text-white' : 'bg-blue-600 hover:bg-blue-700 text-white'}`}>
                <!-- Plus Icon for Add Event Button -->
            ...
            <div class="grid grid-cols-7 gap-px">
                ...
                {:else if daysInMonth.length > 0}
                    {#each daysInMonth as day (day.date.toISOString() + '-' + day.dayNum + '-' + day.events.map(e=>e.id + (e.deadlineTime || '')).join('-'))} <div
                            ...
                            `}
                            role="gridcell" aria-label={`Date ${day.date.toLocaleDateString()}${day.events.length ? ', ' + day.events.length + ' event' + (day.events.length > 1 ? 's' : '') : ''}`}
                        ...
                        >
                            ...
                            {#if day.isCurrentMonth && day.events.length > 0}
                                ...
                                <ul class="mt-0.5 text-[0.6rem] sm:text-[0.7rem] space-y-0.5 overflow-hidden flex-grow">
                                    {#each day.events.slice(0, maxEventsToShow) as event (event.id)}
                                        <li
                                            class={`truncate p-0.5 sm:px-1 sm:py-0.5 rounded text-white leading-tight cursor-pointer hover:opacity-80 transition-opacity`}
                                            style="background-color: {event.color || (isDarkMode ? '#374151' : '#9CA3AF')};"
                                            on:click|stopPropagation={() => openTaskDetailsModal(event.id)}
                                            on:keydown|stopPropagation={(e) => { if (e.key === 'Enter' || e.key === ' ') openTaskDetailsModal(event.id); }}
                                            tabindex="0"
                                            role="button"
                                            aria-label={`View details for ${event.title}`}
                                            title={event.title + (event.deadlineTime ? ` (DL: ${event.deadlineTime})` : '')}
                                        >
                                            {event.title}
                            ...
                            >
                               <!-- Plus icon for day cell button -->
                               <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-2.5 h-2.5 sm:w-3 sm:h-3">
                                  <path d="M10.75 4.75a.75.75 0 00-1.5 0v4.5h-4.5a.75.75 0 000 1.5h4.5v4.5a.75.75 0 001.5 0v-4.5h4.5a.75.75 0 000-1.5h-4.5v-4.5z" />
                               </svg>
                            </button>
                        </div>
  ...
  {#if showEventForm}
    ...
    >
      ...
      >
        ...
        <form method="POST" action={editingEventId ? "?/updateEvent" : "?/addEvent"} use:enhance={eventFormEnhanceCallback} class={`flex-grow flex flex-col overflow-y-auto p-4 sm:p-5 custom-scrollbar space-y-4`}>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
           ...
           <div class="grid grid-cols-1 sm:grid-cols-1 gap-4"> <div> <!-- Wrapped in grid for consistency, though it's a single item -->
  ...
  {#if showTaskDetailsModal && selectedTaskForDetails}
    <div
      ...
      on:click|self={closeTaskDetailsModal} transition:fade={{ duration: 150 }}
      role="dialog" aria-modal="true" aria-labelledby="task-details-title"
    >
      <div
        class={`rounded-lg shadow-xl w-full max-w-md max-h-[90vh] flex flex-col overflow-hidden ${isDarkMode ? 'bg-zinc-800 text-zinc-300' : 'bg-white text-gray-800'}`}
        on:click|stopPropagation
        transition:scale={{ duration: 200, start: 0.95, opacity: 0.5 }}
      >
        <div class={`flex justify-between items-center p-4 sm:p-5 border-b flex-shrink-0 ${isDarkMode ? 'border-zinc-700' : 'border-gray-200'}`}>
          <h3 id="task-details-title" class="text-lg sm:text-xl font-semibold">
            Task Details
          </h3>
          <button type="button" class={`p-1 rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 transition-colors duration-150 ${isDarkMode ? 'text-zinc-400 hover:bg-zinc-700 focus:ring-zinc-600' : 'text-gray-400 hover:bg-gray-100 hover:text-gray-600 focus:ring-gray-400'}`} on:click={closeTaskDetailsModal} aria-label="Close task details">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18 18 6M6 6l12 12" /></svg>
          </button>
        </div>

        <div class="flex-grow overflow-y-auto p-4 sm:p-5 space-y-3 custom-scrollbar">
            <div class="flex items-center mb-2">
                <span class="w-4 h-4 rounded-full mr-2 flex-shrink-0" style="background-color: {selectedTaskForDetails.color || '#3B82F6'};"></span>
                <h4 class="text-xl font-semibold break-words" style="color: {selectedTaskForDetails.color || (isDarkMode ? '#E5E7EB' : '#1F2937')};">
                    {selectedTaskForDetails.title}
                </h4>
            </div>

            {#if selectedTaskForDetails.description}
              <div>
                <p class={`text-sm font-medium mb-0.5 ${isDarkMode ? 'text-zinc-200' : 'text-gray-700'}`}>Description:</p>
                <p class={`text-sm whitespace-pre-wrap p-2 rounded ${isDarkMode ? 'text-zinc-400 bg-zinc-700' : 'text-gray-600 bg-gray-50'}`}>{selectedTaskForDetails.description}</p>
              </div>
            {/if}

            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                    <p class={`text-xs font-medium mb-0.5 ${isDarkMode ? 'text-zinc-300' : 'text-gray-500'}`}>DUE DATE</p>
                    <p class={`text-sm ${isDarkMode ? 'text-zinc-100' : 'text-gray-700'}`}>
                        {selectedTaskForDetails.dueDateISO ? new Date(selectedTaskForDetails.dueDateISO + 'T00:00:00Z').toLocaleDateString(undefined, { year: 'numeric', month: 'long', day: 'numeric', timeZone: 'UTC' }) : 'Not set'}
                    </p>
                </div>

                {#if selectedTaskForDetails.dueTime}
                    <div>
                        <p class={`text-xs font-medium mb-0.5 ${isDarkMode ? 'text-zinc-300' : 'text-gray-500'}`}>DEADLINE TIME</p>
                        <p class={`text-sm ${isDarkMode ? 'text-zinc-100' : 'text-gray-700'}`}>{selectedTaskForDetails.dueTime}</p>
                    </div>
                {/if}
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                    <p class={`text-xs font-medium mb-0.5 ${isDarkMode ? 'text-zinc-300' : 'text-gray-500'}`}>STATUS</p>
                    <span class={`px-2 py-0.5 text-xs font-semibold rounded-full inline-block
                        ${selectedTaskForDetails.status === 'complete' ? (isDarkMode ? 'bg-green-700 text-green-200' : 'bg-green-100 text-green-800') :
                        selectedTaskForDetails.status === 'pending' ? (isDarkMode ? 'bg-yellow-700 text-yellow-200' : 'bg-yellow-100 text-yellow-800') :
                        selectedTaskForDetails.status === 'incomplete' ? (isDarkMode ? 'bg-red-700 text-red-200' : 'bg-red-100 text-red-800') :
                        selectedTaskForDetails.status === 'late' ? (isDarkMode ? 'bg-orange-600 text-orange-100' : 'bg-orange-100 text-orange-700') :
                        (isDarkMode ? 'bg-zinc-600 text-zinc-300' : 'bg-gray-200 text-gray-700')
                        }`}>
                        {selectedTaskForDetails.status.charAt(0).toUpperCase() + selectedTaskForDetails.status.slice(1)}
                    </span>
                </div>

                {#if selectedTaskForDetails.priority}
                    <div>
                        <p class={`text-xs font-medium mb-0.5 ${isDarkMode ? 'text-zinc-300' : 'text-gray-500'}`}>PRIORITY</p>
                        <p class={`text-sm ${isDarkMode ? 'text-zinc-100' : 'text-gray-700'}`}>{typeof selectedTaskForDetails.priority === 'string' ? selectedTaskForDetails.priority.charAt(0).toUpperCase() + selectedTaskForDetails.priority.slice(1) : selectedTaskForDetails.priority}</p>
                    </div>
                {/if}
            </div>

            {#if selectedTaskForDetails.createdAtISO}
              <div>
                <p class={`text-xs font-medium mb-0.5 ${isDarkMode ? 'text-zinc-300' : 'text-gray-500'}`}>CREATED</p>
                <p class={`text-sm ${isDarkMode ? 'text-zinc-400' : 'text-gray-600'}`}>{new Date(selectedTaskForDetails.createdAtISO).toLocaleString(undefined, {dateStyle: 'medium', timeStyle: 'short'})}</p>
              </div>
            {/if}
        </div>

        <div class={`flex justify-end p-4 sm:p-5 border-t flex-shrink-0 ${isDarkMode ? 'bg-zinc-750 border-zinc-700' : 'bg-gray-50 border-gray-200'}`}>
          <button type="button" class={`w-full sm:w-auto px-4 py-2 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 transition-colors duration-150 ${isDarkMode ? 'bg-zinc-600 text-zinc-300 hover:bg-zinc-500 focus:ring-zinc-500' : 'bg-gray-200 text-gray-800 hover:bg-gray-300 focus:ring-gray-400'}`} on:click={closeTaskDetailsModal}>
            Close
          </button>
        </div>
      </div>
